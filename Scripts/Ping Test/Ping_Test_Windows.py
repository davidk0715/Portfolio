...
def test_default(default_gateway):
    ...
    process = subprocess.Popen(['ping','-n' ,'1', str(default_gateway)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # With 4 packets
...
def test_remote_host(remote_ip):
    ...
    # Creates the ping command
    process = subprocess.Popen(['ping','-n' ,'1', str(remote_ip)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # With 4 packets
    stdout, stderr = process.communicate()
    returncode = process.returncode
...
def test_dns(google_ip):
    ...
    # Creates the ping command
    process = subprocess.Popen(['ping','-n' ,'1', str(google_ip)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    # With 4 packets
    stdout, stderr = process.communicate()
    returncode = process.returncode
